...
class xFuserTransformerBaseWrapper(xFuserModelBaseWrapper, metaclass=ABCMeta):
    ...
    def _split_transformer_blocks(
        ...
        if attn_layer_num_for_pp is not None:
            ...
            # give more blocks to the later stage s as the first stage has more processings to do
            blocks_dict = [
                (
                    i * num_blocks_per_stage + max(0, i - (pp_world_size - remainder)),
                    (i + 1) * num_blocks_per_stage + max(0, (i+1) - (pp_world_size - remainder)),
                )
                for i in range(pp_world_size)
            ]
            stage_block_start_idx, stage_block_end_idx = blocks_dict[pp_rank]
